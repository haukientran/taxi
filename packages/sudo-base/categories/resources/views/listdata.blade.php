			...
			<div class="card-header">
				...
				<div class="float-right action-container">
					...
					@if (isset($data['no_trash']) && $data['no_trash'] == true)
                    	<a href="{{ route('admin.'.$table_name.'.index', ['trash' => 'true']) }}" class="btn btn-sm btn-default"><i class="fas fa-trash mr-1"></i>@lang('Translate::table.trash_url')</a>
                    @endif
									...
									@foreach ($data['table_generate'] as $generate)
										...
										@if ($generate['type'] == 'lang')
											@if (isset($has_locale) && $has_locale == true && !empty(config('app.language') ?? []))
												<th class="lang" style="width: calc(26px * {{count(config('app.language') ?? [])}} + 40px)">
