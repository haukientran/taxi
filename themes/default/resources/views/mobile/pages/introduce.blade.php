...
<main class="introduce">
    <div class="container">
        <section class="introduce-banner w-100">
            ...
            {{-- <div class="introduce-banner__bottom">
        ...
        <div class="container">
            <div class="story-content w-100">
				<div class="story-content__left">
                    <h2 class="story-title fs-16 lh-24 text-up f-w-b color_white mb-15">Câu chuyện thương hiệu</h2>
                    <h3 class="introduce-text fs-30 f-w-b lh-46 color_white">{!! $setting_introduce['story_title'] ?? '' !!}  </h3>
                    <p class="fw-600 lh-20 color_white introduce-des">{!! $setting_introduce['story_des'] ?? '' !!} </p>
				</div>
				<div class="story-content__right">
					@include('Default::general.components.image', [
           	            'src' => resizeWImage($setting_introduce['story_image'] ?? '/assets/images/image-thuong-hieu.png', 'w600'),
						'width' => '397px',
						'height' => '562px',
						"lazy"   => true,
						"title" =>''
					])
                    <div class="ck ck-reset ck-editor ck-rounded-corners story_detail" role="application" dir="ltr">
                        <div class="ck-content">
                            {!! replaceCkeditor($setting_introduce['story_detail'] ?? '' )!!}
                        </div>
                    </div>
				</div>
			</div>
    ...
    <section class="achievement" id="achievement">
        <div class="introduce-background">
            @include('Default::general.components.image', [
                'src' =>  resizeWImage($setting_introduce['achievement_banner'] ??  '/assets/images/bg-thanh-tuu-1.png' , ''),
                'width' => '100%',
                'height' => '100%',
                "lazy"   => true,
                "title" =>''
        ...
        <div class="container">
            <div class="achievement-content w-100">
                <h2 class="introduce-title fs-30 lh-46 f-w-b color_white mb-30 w-100">{{ $setting_introduce['achievement_title'] ?? 'Thành tựu của chúng tôi' }}</h2>
                <div class="achievement-content__list">
                    @if(isset($setting_introduce['achievement']['title']) && count($setting_introduce['achievement']['title']) > 0)
                        @foreach($setting_introduce['achievement']['title'] as $key => $title)
                            <div class="item">
                                <strong class="f-w-b fs-20 lh-24 text-center color_white">{{ $setting_introduce['achievement']['name'][$key] ?? '' }}</strong>
    ...
    <section class="core_value">
        ...
        <div class="container">
            <div class="core_value-content w-100">
                <h2 class="introduce-title fs-30 lh-46 f-w-b color_white">Sứ mệnh, <br> giá trị cốt lõi</h2>
                <div class="core_value-content__list flex mt-30">
                    @if(isset($setting_introduce['core_value']['name']) && count($setting_introduce['core_value']['name']) > 0)
                        @foreach($setting_introduce['core_value']['name'] as $key => $name)
                            <div class="item">
                                <h3 class="f-w-b fs-20 lh-24 color_white"><span>{{ $key+1 }}</span>. {{ $name }}</h3>
    ...
    @include('Default::mobile.layouts.team',['title'=>$setting_introduce['team_title'] ?? '<span> Được dẫn dắt</span> <br> bới chuyên gia','personnels'=>$personnels])
    <section class="story" id="story">
        <div class="introduce-background">
        ...
        <div class="container">
            <div class="story-content w-100">
				<div class="story-content__left">
                    <h2 class="introduce-text fs-30 f-w-b lh-36 color_white">Không ngừng mang lại giá trị cho <span>đội ngũ</span></h2>
                    <p class="fw-600 lh-20 color_white introduce-des">{!! $setting_introduce['value_title'] ?? ''!!} <br><span class="fs-14 fw-400 color_white lh-18">{!! $setting_introduce['value_des'] ?? '' !!}</span> </p>
				</div>
				<div class="story-content__right">
					@include('Default::general.components.image', [
                    ...
                    <div class="ck ck-reset ck-editor ck-rounded-corners story_detail  mt-20" role="application" dir="ltr">
                        <div class="ck-content">
                            {!! replaceCkeditor($setting_introduce['value_detail'] ?? '' )!!}
                        </div>
                    </div>
				</div>
			</div>
    ...
    <section id="evaluate" style="background :{{ isset($config_general['color_backgroud']) ? $config_general['color_backgroud'] : '#363636' }}">
        <div class="evaluate-container">
            <div class="evaluate-list s-wrap" id="evaluate-slide">
                <h2 class="section-title evaluate-title fs-30 lh-36 f-w-b color_white">Không ngừng <br> mang lại giá trị cho KHÁCH HÀNG</h2>
                <div class="s-content mt-20">
