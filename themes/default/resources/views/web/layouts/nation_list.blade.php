    ...
    <div class="nation-item">
        <div class="nation-item__thumbnail">
            <a href="{{ $arboard->getUrl() ?? '' }}" aria-label="{{ $arboard->name ?? '' }}">
            @include('Default::general.components.image', [
                    'src' => resizeWImage($arboard->banner_image ?? '', 'w400'),
                    'width' => '380px',
                    'height' => '150px',
                    'lazy'   => true,
        ...
        <div class="nation-item__content">
            ...
            <a href="{{ $arboard->getUrl() ?? '' }}" class="btn btn-primary w-100 nation-item__see color_white" aria-label="Xem chi tiết" title="Xem chi tiết">Xem chi tiết</a>
        </div>
    </div>
    @endforeach
